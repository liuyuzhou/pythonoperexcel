...
# 按指定的端点值划分区间
df_cut = pd.cut(df['库存量'], bins=range(100, 1001, 100))
# 统计各个区间的数据
...
# 按指定的端点值划分区间
n, bins, patches = plt.hist(df['库存量'], bins=range(100, 1001, 100),
...
try:
    ...
    if select_sheet:
        ...
        # 将绘制的直方图转换为图片并写入工作表
        worksheet.pictures.add(fig, name='图2', update=True, left=400, top=200)
        # 根据数据内容自动调整工作表的行高和列宽
        worksheet.autofit()
        # 保存工作薄
        workbook.save(os.path.join(full_path, '库存统计2.xlsx'))
